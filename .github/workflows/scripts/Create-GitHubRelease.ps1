...
# ═══════════════════════════════════════════════════════════════════════════
# 📦 Load Bootstrap Functions from external script
# ═══════════════════════════════════════════════════════════════════════════
# Provides: Repair-PSResourceGetCase, Install-PackageRepoProvider, Install-ModuleViaPackageRepoProvider
# See: Install-PackageRepoProviderBootstrap.ps1 for details
...
try {
    ...
    # Step 1: Bootstrap PackageRepoProvider (includes GitHub provider dependency)
    Install-PackageRepoProvider -Token $githubToken -Owner $owner

    # Step 2: Install Smartagr via the bootstrapped provider
    Install-ModuleViaPackageRepoProvider -ModuleName 'K.PSGallery.Smartagr' -Token $githubToken -Owner $owner

    Write-Output "✅ K.PSGallery.Smartagr installed and ready"
}
catch {
    Write-Error "❌ Failed to install Smartagr module: $($_.Exception.Message)"
